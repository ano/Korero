...
$dialects_edit = NULL; // Initialize page object first

class cdialects_edit extends cdialects {
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "")
			$this->Page_Terminate("dialectslist.php"); // Invalid key, return to list
	...
	function ValidateForm() {
		...
		if (in_array("words", $DetailTblVar) && $GLOBALS["words"]->DetailEdit) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// language_id
			$this->language_id->SetDbValueDef($rsnew, $this->language_id->CurrentValue, 0, $this->language_id->ReadOnly);

			// dialects
			$this->dialects->SetDbValueDef($rsnew, $this->dialects->CurrentValue, "", $this->dialects->ReadOnly);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					...
					if (in_array("words", $DetailTblVar) && $GLOBALS["words"]->DetailEdit) {
						if (!isset($GLOBALS["words_grid"])) $GLOBALS["words_grid"] = new cwords_grid(); // Get detail page object
						$EditRow = $GLOBALS["words_grid"]->GridUpdate();
					}
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("words", $DetailTblVar)) {
				...
				if ($GLOBALS["words_grid"]->DetailEdit) {
					$GLOBALS["words_grid"]->CurrentMode = "edit";
					$GLOBALS["words_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($dialects_edit)) $dialects_edit = new cdialects_edit();

// Page init
$dialects_edit->Page_Init();

// Page main
$dialects_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$dialects_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var dialects_edit = new ew_Page("dialects_edit");
dialects_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = dialects_edit.PageID; // For backward compatibility

// Form object
var fdialectsedit = new ew_Form("fdialectsedit");

// Validate form
fdialectsedit.Validate = function() {
...
fdialectsedit.Form_CustomValidate = 
...
fdialectsedit.ValidateRequired = true;
<?php } else { ?>
fdialectsedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fdialectsedit.Lists["x_language_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x__language","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $dialects_edit->ShowPageHeader(); ?>
<?php
$dialects_edit->ShowMessage();
?>
<form name="fdialectsedit" id="fdialectsedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="dialects">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_dialectsedit" class="table table-bordered table-striped">
<?php if ($dialects->id->Visible) { // id ?>
	<tr id="r_id">
		<td><span id="elh_dialects_id"><?php echo $dialects->id->FldCaption() ?></span></td>
...
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($dialects->id->CurrentValue) ?>">
<?php echo $dialects->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php
	if (in_array("words", explode(",", $dialects->getCurrentDetailTable())) && $words->DetailEdit) {
?>
